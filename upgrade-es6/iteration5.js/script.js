...
const leagueGame = streamers.filter(streamer => streamer.gameMorePlayed == 'League of Legends');
...
const nameStream = streamers.filter(streamer => streamer.name.includes('u'));
console.log(nameStream);
...
const input = document.querySelector('.toFilterStreamers');
 ...
 function streamersFilter(event) {
    const streamFilter = streamers.filter(streamer => streamer.name.toLocaleLowerCase().includes(input.value.toLowerCase()));
     console.log(streamFilter);
}

input.addEventListener('keyup', streamersFilter);
...
const btn = document.querySelector('.toShowFilterStreamers');

btn.addEventListener('click', streamersFilter);
